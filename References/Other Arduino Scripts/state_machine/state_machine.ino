...
// Signal Processing Variable Definitions
Queue<uint16_t> queue = Queue<uint16_t>(5);
volatile uint16_t signal_threshold = 100;
volatile uint16_t baseline = 500;
volatile uint16_t noise_threshold = 5;
...
void loop()
{
  ...
  {
    // Processing (this grabs the ADC reading from the most recent of 8-batched readings)
    const uint16_t signal = adc_now;
    // Serial.print(String(signal) + "\n");
    uint16_t prior = queue.pop();
    queue.push(signal);
    uint16_t upper = baseline + signal_threshold;
    uint16_t lower = baseline - signal_threshold;

    if (signal < upper && signal > lower) {
      if (abs(signal - prior) < noise_threshold) {
        baseline = (signal + prior) / 2;
      }
    } else if (signal > upper && signal < prior) {
      output = prior - baseline;
      Serial.print(String(output));
    } else if (signal < lower && signal > prior) {
      output = prior - baseline;
      Serial.print(String(output));
    }
